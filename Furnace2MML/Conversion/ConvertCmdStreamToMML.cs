...
{
    public static void ConvertNoteOn(FurnaceCommand cmd, int tickLen, int defaultOct, StringBuilder curOrderSb)
    {
        var noteNum = cmd.Value1;
        var mmlNote = CmdStreamToMMLUtil.GetMMLNote(noteNum, defaultOct);
    ...
    public static void ConvertPortamento(List<FurnaceCommand> cmdList, int curCmdIdx, int defaultOct,  StringBuilder curOrderSb)
    {
	    ...
	    var isEndOfPorta = nextCmd.CmdType.Equals("HINT_LEGATO") || nextCmd.CmdType.Equals("NOTE_OFF");
	    ...
	    var prevMMLNote = CmdStreamToMMLUtil.GetMMLNote(prevCmd.Value1, defaultOct); 
        var curMMLNote = CmdStreamToMMLUtil.GetMMLNote(curCmd.Value1, defaultOct);
	    ...
	    /*
	    if(isEndOfPorta)
			curOrderSb.Append($"&{{{prevMMLNote} {curMMLNote}}}").Append($"&{curMMLNote.ToString().Trim(['<', '>'])}").AppendFracLength(portaLength);
	    else
			curOrderSb.Append($"&{{{prevMMLNote} {curMMLNote}}}").AppendFracLength(portaLength);
    */
    }
