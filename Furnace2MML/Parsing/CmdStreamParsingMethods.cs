    ...
    public void FixRetriggerCommands()
    {
        ...
        for(var chNum = 0; chNum < 9; chNum++) {
            ...
            for(var i = 1; i < noteCmdChLen; i++) {
                var prevNoteOnCmd = GetFirstCertainCmd(noteCmdChList, i, ["NOTE_ON"], "backward");
                var curCmd = noteCmdChList[i];
                ...
                if(curCmd.Tick == prevNoteOnCmd.Tick) {
                    noteCmdChList.Remove(curCmd, ref noteCmdChLen);
                    i--;
                } else {
                    noteCmdChList[i] = new FurnaceCommand(curCmd.Tick, prevNoteOnCmd);  // FurnaceCommand(int tick, FurnaceCommand otherCmd): Copy otherCmd except tick
                }
            }
        }
    }
